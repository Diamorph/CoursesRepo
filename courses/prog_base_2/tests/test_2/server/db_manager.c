...
void db_getScientists(db_t * self,list_t * list1)
{
    scientist_t * scientist = malloc(sizeof(scientist_t*));
    sqlite3_stmt * stmt = NULL;
    sqlite3_prepare_v2(self->db, "SELECT * FROM Scientists;", -1, &stmt, 0);
while (1)
{
    int rc = sqlite3_step(stmt);
    if (SQLITE_ERROR == rc)
...
{
cJSON * jScientist = cJSON_CreateObject();
cJSON_AddItemToObject(jScientist, "name", cJSON_CreateString(self->name));
cJSON_AddItemToObject(jScientist, "surname", cJSON_CreateString(self->surname));
cJSON_AddItemToObject(jScientist, "birthday", cJSON_CreateString(self->birthdate));
cJSON_AddItemToObject(jScientist, "articles", cJSON_CreateNumber(self->articles));
cJSON_AddItemToObject(jScientist, "articles", cJSON_CreateNumber(self->articles));
cJSON_AddItemToObject(jScientist, "books", cJSON_CreateNumber(self->books));
char * jsonString = cJSON_Print(jScientist);
return jsonString;
}
