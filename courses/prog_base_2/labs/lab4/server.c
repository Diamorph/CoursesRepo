...
void server_answer(http_request_t req, socket_t * clientSocket, list_t * teacherList) {
    ...
    if (!strcmp(req.uri, "/Scientist/api"))
    {
    ...
    else if (strncmp(req.uri, "/Scientist/api/", 15) == 0)
    {
        if(!strcmp(req.method, "GET"))
...
void server_GET_html(socket_t* clientSocket, list_t * teacherList) {
    ...
    for(int i = 0; i < count; i++) {
        ...
        sprintf(text, "<p><a href=""/Scientist/%i"">%s %s</a></p>", i, teacher_getName(temp), teacher_getBirthdate(temp));
        strcat(buf,text);
    }
