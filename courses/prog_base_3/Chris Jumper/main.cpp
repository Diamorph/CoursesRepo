...
public:
	...
	void control_1(float time, float dx){
		if (dx < 0){
			...
			if (CurrentFrame1 > 4) CurrentFrame1 -= 3;
	...
	void update(float time)
	{
		if (name == "EasyEnemy"){//для персонажа с таким именем логика будет такой
			...
			moveTimer += time;if (moveTimer>3000){ dx = dx *(-1); moveTimer = 0; }//меняет направление примерно каждые 3 сек
			control(time, dx);
...
bool startGame(RenderWindow &window, int &numberLevel){
	...
	Image win_im; //создаем объект Image (изображение)
	win_im.loadFromFile("images/you_win.png");//загружаем в него файл

	Texture win;//создаем объект Texture (текстура)
	win.loadFromImage(win_im);//передаем в него объект Image (изображения)

	Sprite win_s;//создаем объект Sprite(спрайт)
	win_s.setTexture(win);//передаём в него объект Texture (текстуры)
	win_s.setPosition(50, 25);//задаем начальные координаты появления спрайта
	...
	while (window.isOpen())
	{
		...
		if (Keyboard::isKeyPressed(Keyboard::F1)) { return true; }//если таб, то перезагружаем игру
